pipeline {
    ...
    stage('docker-login'){
        ...
        script {
                ...
                build propagate: false, job: 'kustomize-jenkins', parameters: [string(name: 'Enviroment_name', value: "${env.IMAGE_VERSION}.${env.BUILD_NUMBER}"),
                string(name: 'Release_no', value: "${env.IMAGE_VERSION}.${env.BUILD_NUMBER}"]
        }
        }
    }
    }
